...
export function createDebugContext(env, { categories }: {categories: any[]} = { categories : [] }) {
    return { [debugContextSymbol]: new DebugContext(env, categories) };
}

export function useOwnDebugContext({ categories }: {categories: any[]} = { categories : [] }) {
    useSubEnv(createDebugContext(useEnv(), { categories }));
}

export function useEnvDebugContext() {
    const debugContext = useEnv()[debugContextSymbol.toString()];
    if (!debugContext) {
