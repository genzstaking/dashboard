...
import { View, ViewNotFoundError } from "../../views/view";
...
function parseActiveIds(ids: any) {
    const activeIds: any[] = [];
...
function makeActionManager(env) {
    ...
    function _getActionMode(action) {
        ...
        if (controllerStack.some((c:any) => c.action.target === "fullscreen")) {
    ...
    function _getViewInfo(view, action, views, props = {}) {
        ...
        const viewProps: any = Object.assign({}, props, {
            resId: undefined,
     ...
     */
    function _computeStackIndex(options) : number {
        let index: number = 0;
     ...
     */
    async function _updateUI(controller, options: any = {}) {
        let resolve;
        let reject;
        ...
        const index: number = _computeStackIndex(options);
        ...
        class ControllerComponent extends Component {
            ...
            Component: any;
            titleService = useService("title");
            __beforeLeave__: CallbackRecorder;
            __getGlobalState__: CallbackRecorder;
            __getLocalState__: CallbackRecorder;
            setup() {
        ...
        let nextDialog: any | undefined = undefined;
        if (action.target === "new") {
            //cleanDomFromBootstrap();
